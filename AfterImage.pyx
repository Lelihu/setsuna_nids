...
cdef class incStat:
    ...
    cdef insert(self, double v, double t=0):  # v is a scalar, t is v's arrival the timestamp
        if self.isTypeDiff:
            v = t - self.lastTimestamp if t - self.lastTimestamp > 0 else 0
        self.processDecay(t)
    ...
    def predict(self, double t):
        if self.Qt.n < 2:
            if self.Qt.n == 1:
                ...
                return 0
        if (t - self.Qt.get_last())/(self.Qt.get_mean_diff() + 1e-10) > 10:
            return self.Qv.get_last()
        cdef double yp
...
cdef class incStatDB2(incStatDB):
    ...
    def register(self, str ID, double Lambda=1,double init_time=0, int isTypeDiff=False, str dir='o'):
        Lambda = self.get_lambda(Lambda)
